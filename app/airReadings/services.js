...
exports.getALatestAirReading = function (nodeID, result) {
    const sqlQuery = `SELECT id, ROUND(ch4, 2) as ch4, ROUND(co, 2) AS co, ROUND(dust, 2) AS dust, ROUND(humidity, 2) AS humidity, ROUND(nh3, 2) AS nh3, ROUND(no2, 2) AS no2, ROUND(co2, 2) AS co2, ROUND(temperature, 2) AS temperature, to_char(created_time , 'YYYY-MM-DD HH24:MI') AS created_time FROM "AirReading" WHERE node_id=${nodeID} ORDER BY created_time DESC`;
    try {
        pool.query(sqlQuery, [], (err, res) => {
            if (err) {
                logger.error('Error: ', err.stack);
...
exports.getAllAirReadingWithPagination = function (page, pageSize, sortingName, sortingOrder, result) {
    ...
    const sqlQuery = `SELECT id, ROUND(ch4, 2) as ch4, ROUND(co, 2) AS co, ROUND(dust, 2) AS dust, ROUND(humidity, 2) AS humidity, ROUND(nh3, 2) AS nh3, ROUND(no2, 2) AS no2, ROUND(co2, 2) AS co2, ROUND(temperature, 2) AS temperature, to_char(created_time , 'YYYY-MM-DD HH24:MI') AS created_time FROM "AirReading" ${sortingQuery} LIMIT ${pageSize} OFFSET ${page * pageSize} `;
